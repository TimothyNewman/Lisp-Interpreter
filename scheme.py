...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(procedure, MacroProcedure):
            ...
            eval = macro.apply_macro(rest, env)
            return scheme_eval(eval, env)
        else:
...
def eval_all(expressions, env):
    ...
    while expressions is not nil:
        nil_checker = expressions.second is nil
        value = scheme_eval(expressions.first, env, nil_checker)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        return_value = scheme_eval(expressions.second.first, env)
        env.define(target, return_value)
        return target
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        return_expr = do_lambda_form(expressions, env)
        return_expr.formals = target.second
        env.define(target.first, return_expr)
        return target.first
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions.second is nil:
        return val
    elif scheme_truep(val):
        return do_and_form(expressions.second, env)
    elif not scheme_truep(val):
...
def make_let_frame(bindings, env):
    ...
    symbol_bindings, val_bindings = nil, nil
    while bindings is not nil:
        check_form(bindings.first, 2, 2)
        symbol_bindings = Pair(bindings.first.first, symbol_bindings)
        val_bindings = Pair(scheme_eval(bindings.first.second.first, env), val_bindings)
        check_formals(symbol_bindings)
        bindings = bindings.second
    return env.make_child_frame(symbol_bindings, val_bindings)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        mac = MacroProcedure(formals, expressions.second, env)
        mac.formals = target.second
        env.define(target.first, mac)
        return target.first
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        optimize_tail_calls(original_scheme_eval)
